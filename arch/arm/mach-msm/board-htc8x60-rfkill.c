...
static void msm8x60_config_bt_on(void)
{
	printk(KERN_INFO "[BT]-- R ON --\n");

	/* set bt on configuration*/
	config_bt_table(msm8x60_bt_on_table,
				ARRAY_SIZE(msm8x60_bt_on_table));
	mdelay(5);
...
static void msm8x60_config_bt_off(void)
{
	...
	config_bt_table(msm8x60_bt_off_table,
				ARRAY_SIZE(msm8x60_bt_off_table));
	mdelay(2);
...
{
	if (!blocked)
		msm8x60_config_bt_on();
	else
		msm8x60_config_bt_off();

	return 0;
}

static struct rfkill_ops msm8x60_rfkill_ops = {
	.set_block = bluetooth_set_power,
};

static int msm8x60_rfkill_probe(struct platform_device *pdev)
{
	int rc = 0;
...
static int msm8x60_rfkill_remove(struct platform_device *dev)
{
...
static struct platform_driver msm8x60_rfkill_driver = {
	.probe = msm8x60_rfkill_probe,
	.remove = msm8x60_rfkill_remove,
	.driver = {
		.name = "msm8x60_rfkill",
		.owner = THIS_MODULE,
	},
